...
const Paint = () => {
  ...
  const [brush, setBrush] = useState<'square' | 'round'>('round');
  ...
  const brushHundler = (value: 'round' | 'square') => (e: React.FormEvent<HTMLButtonElement>) => {
    setBrush(value);
  };

  const getBorderColorForBrashButtons = (value: 'butt' | 'round' | 'square', brash: string) => {
    if (value === brash) return 'border-white';
    return 'border-orange-500';
  };

  return (
    <div className="w-full h-full mt-6 flex justify-center rounded-lg shadow-xl shadow-orange-800">
      ...
      <Canvas brushSize={brushSize} color={color} brush={brush}/>
    </div>
  );
};
